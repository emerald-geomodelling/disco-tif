...
def rast_dat_to_QGIS_lut(cmap, data_breaks, dtype, output_GIS_lut_path, path_to_orig_geotiff=None, short_file=True):
...
Input Parameters:
    ...
    with open(qgisfile, 'w') as outlut:
        outlut.write(f"# EMerald Generated Color Map Export File for {path_to_orig_geotiff}\n")
        outlut.write("INTERPOLATION:INTERPOLATED\n")
        for ii in range(0, len(colors_rgba)):
            outlut.write(f"{colors_rgba.loc[ii,'data_val'].astype(dtype)},{np.round(colors_rgba.loc[ii,'r']).astype(int)},{np.round(colors_rgba.loc[ii,'g']).astype(int)},{np.round(colors_rgba.loc[ii,'b']).astype(int)},{np.round(colors_rgba.loc[ii,'a']).astype(int)},{colors_rgba.loc[ii,'data_val'].astype(dtype)}\n")
        
    print(f"\nGIS software compatible color look up table successfully written to: \n\t- {qgisfile}")


######################################
def rast_dat_to_single_channel_rgba_luts(cmap, data_breaks, dtype, no_data_value, lut_outpath_base, short_file=True):
...
def rast_dat_to_multi_channel_rgba_lut(cmap, data_breaks, dtype, no_data_value, lut_outpath_base, single_band_tiff_path=None, write_lut_files=False, short_file=True):
...
Input Parameters:
    ...
    with open(tname, 'w') as outlut:
        for ii in range(0, len(colors_rgba)):
            outlut.write(f"{colors_rgba.loc[ii,'data_val'].astype(dtype)},{np.round(colors_rgba.loc[ii,'r']).astype('uint8')},{np.round(colors_rgba.loc[ii,'g']).astype('uint8')},{np.round(colors_rgba.loc[ii,'b']).astype('uint8')},{np.round(colors_rgba.loc[ii,'a']).astype('uint8')}\n")
    
    print(f"\nmulti-channel LUT file mapping data to color values has been written to: \n\t- {tname}")
    

def uint8dat_to_single_channel_rgba_luts(cmap, data_breaks, dtype, no_data_value, lut_outpath_base, short_file=True):
...
def uint8dat_to_multi_channel_rgba_lut(cmap, data_breaks, dtype, no_data_value, lut_outpath_base, short_file=True):
...
Input Parameters:
    ...
    with open(tname, 'w') as outlut:
        for ii in range(0, len(colors_rgba)):
            outlut.write(f"{colors_rgba.loc[ii,'data_uint8_val'].astype('uint8')},{np.round(colors_rgba.loc[ii,'r']).astype('uint8')},{np.round(colors_rgba.loc[ii,'g']).astype('uint8')},{np.round(colors_rgba.loc[ii,'b']).astype('uint8')},{np.round(colors_rgba.loc[ii,'a']).astype('uint8')}\n")
    
    print(f"\nmulti-channel LUT file mapping data to color values has been written to: \n\t- {tname}")


def data_to_uint8_lut(data_breaks, no_data_value, lut_outpath_base):
